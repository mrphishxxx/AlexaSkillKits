"""
Simple Alexa skill that read out the summary section from wikipedia on 
the subject that user wanted to know about.
"""
...
def on_intent(intent_request, session):
    ...
    elif intent_name == "TellMeSomething":
...
def get_welcome_response():
    ...
    session_attributes = {}
    card_title = "Tell Me Something About"
...
def get_information(intent, session):
    """
    The main function that looks up the summary infomation about a subject from wikipedia.
    """
    card_title = "Tell Me Something About"
    session_attributes = session.get('attributes', {})
    should_end_session = False
    ...
    try: 
    	subject = intent['slots']['Thing']['value'] # should be 'about obama' or 'is jet plane'
    	# subject = ' '.join(subject.split()[1:])  # this is to get around with the AMAZON.LITERAL usage
    	wiki_summary = wikipedia.summary(subject)
    	speech_output = "Here is what I found out about " + subject + wiki_summary
        print(wiki_summary)
